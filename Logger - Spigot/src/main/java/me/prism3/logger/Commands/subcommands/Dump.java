...
public class Dump implements SubCommand {
    ...
    private void pastebinExecution(Player player) throws IOException {
        ...
        String config = Files.asCharSource(new File(dataFolder + "/config.yml"), StandardCharsets.UTF_8).read();
        String discord = Files.asCharSource(new File(dataFolder + "/discord.yml"), StandardCharsets.UTF_8).read();
        String messages = Files.asCharSource(new File(dataFolder + "/messages/" + selectedLang + ".yml"), StandardCharsets.UTF_8).read();
        String latest = Files.asCharSource(new File("logs/latest.log"), StandardCharsets.UTF_8).read();
        Pastebin.PasteRequest request = new Pastebin.PasteRequest(dotenv.get("PASTEBIN_API"), config + " \n\n\n\nDISCORD CONFIG\n\n\n\n " + discord + " \n\n\n\nMESSAGES PART\n\n\n\n " + messages + " \n\n\n\nLATEST LOG PART\n\n\n\n " + latest);
