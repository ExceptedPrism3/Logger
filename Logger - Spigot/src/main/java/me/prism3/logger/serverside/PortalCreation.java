        ...
        if (!event.isCancelled() && this.main.getConfig().getBoolean("Log-Server.Portal-Creation")) {
            ...
            if (Data.isLogToFiles) {
                ...
                try {
                    ...
                    out.write(Objects.requireNonNull(Messages.get().getString("Files.Server-Side.Portal-Creation")).replaceAll("%time%", Data.dateTimeFormatter.format(ZonedDateTime.now())).replaceAll("%world%", worldName).replaceAll("%material%", String.valueOf(reason)) + "\n");
                    out.close();
