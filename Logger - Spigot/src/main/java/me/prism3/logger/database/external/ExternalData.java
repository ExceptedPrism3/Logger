...
public class ExternalData {
    ...
    public static ResultSet getMessages(String playerName, int offset) {
        ...
        try {
            Connection connection = plugin.getExternal().getHikari().getConnection();
            final PreparedStatement getStatement = connection.prepareStatement(
                    "SELECT message, id, date FROM player_chat WHERE player_name=? ORDER BY id DESC LIMIT 10 OFFSET ?");
    ...
    public static String getMessagesCount(String playerName) {
        try (
                Connection connection = plugin.getExternal().getHikari().getConnection();

        ) {
            final PreparedStatement getStatement = connection.prepareStatement(
                    "SELECT COUNT(*) as total FROM player_chat WHERE player_name = ?");
            getStatement.setString(1, playerName);
