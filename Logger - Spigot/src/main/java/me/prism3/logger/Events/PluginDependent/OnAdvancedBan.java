        ...
        if (this.main.getConfig().getBoolean("Log-Extras.AdvancedBan")) {
                ...
                if (Data.isStaffEnabled) {
                    if (player.hasPermission(Data.loggerStaffLog)) {
                        ...
                        if (!Objects.requireNonNull(Messages.get().getString("Discord.Extras.AdvancedBan")).isEmpty()) {
                            ...
                            Discord.staffChat(player, Objects.requireNonNull(Messages.get().getString("Discord.Extras.AdvancedBan")).replace("%time%", Data.dateTimeFormatter.format(ZonedDateTime.now())).replace("%executor%", executor).replace("%executed_on%", executed_on).replace("%reason%", reason).replace("%expiration%", String.valueOf(expirationDate)).replace("%type%", type), false);
                        ...
                        try {
                            ...
                            out.write(Objects.requireNonNull(Messages.get().getString("Files.Extras.AdvancedBan")).replace("%time%", Data.dateTimeFormatter.format(ZonedDateTime.now())).replace("%executor%", executor).replace("%executed_on%", executed_on).replace("%reason%", reason).replace("%expiration%", String.valueOf(expirationDate)).replace("%type%", type) + "\n");
                            out.close();
            ...
            if (player == null) { // This is essential when performed by the console
                ...
                if (!Objects.requireNonNull(Messages.get().getString("Discord.Extras.AdvancedBan")).isEmpty()) {
                    ...
                    Discord.advancedBan(Objects.requireNonNull(Messages.get().getString("Discord.Extras.AdvancedBan")).replace("%time%", Data.dateTimeFormatter.format(ZonedDateTime.now())).replace("%executor%", executor).replace("%executed_on%", executed_on).replace("%reason%", reason).replace("%expiration%", String.valueOf(expirationDate)).replace("%type%", type), false);
                }
