...
public class SQLiteData {
    ...
    public void emptyTable() {
        ...
        try {
            ...
            // Player Side Part
            final PreparedStatement playerChat = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM player_chat_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerCommands = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM player_commands_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerLogin = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM player_login_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerLeave = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM player_leave_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            // Server Side Part
            final PreparedStatement serverReload = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM server_reload_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement serverStart = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM server_start_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement serverStop = plugin.getSqLite().getConnection().prepareStatement("DELETE FROM server_stop_proxy WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
