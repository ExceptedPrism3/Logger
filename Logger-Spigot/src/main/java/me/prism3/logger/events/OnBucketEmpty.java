        ...
        if (!event.isCancelled()) {
            ...
            if (Data.isLogToFiles) {
                ...
                if (Data.isStaffEnabled && player.hasPermission(loggerStaffLog)) {
                    ...
                    try (final BufferedWriter out = new BufferedWriter(new FileWriter(FileHandler.getBucketEmptyFolder(), true))) {

                        out.write(this.main.getMessages().get().getString("Files.Bucket-Empty").replace("%time%", Data.dateTimeFormatter.format(ZonedDateTime.now())).replace("%world%", worldName).replace("%player%", playerName).replace("%x%", String.valueOf(x)).replace("%y%", String.valueOf(y)).replace("%z%", String.valueOf(z)).replace("%bucket%", bucket).replace("%uuid%", playerUUID.toString()) + "\n");
