...
import me.prism3.logger.utils.UPotion;
...
public enum UMaterial {
    ...
    UMaterial(String... names) { this(0, names); }
    ...
    UMaterial(String name, int data) {
        ...
        this.material = m;
        this.itemStack = this.version.contains("1.8") || this.version.contains("1.9") || this.version.contains("1.10") || this.version.contains("1.11") || this.version.contains("1.12") ? new ItemStack(m, 1, (byte) data) : new ItemStack(m);
    }
    ...
    UMaterial(String name, int spoofedData, Color color) {
        ...
        final LeatherArmorMeta lam = (LeatherArmorMeta) this.itemStack.getItemMeta();
        lam.setColor(color);
        this.itemStack.setItemMeta(lam);
    }

    UMaterial(String name, int data, UPotion pd) { this.doPotion(this.setItemStack(name, data), pd); }

    UMaterial(UPotion pd, int data, String... names) { this.doPotion(this.setItemStack(data, names), pd); }

    UMaterial(String name, Enchantment enchant, int level) { this.doEnchantmentBook(setItemStack(0, name), enchant, level); }
    ...
    UMaterial(int data, int returnData, String... names) {
        ...
        final Material m = getMaterialName();
        this.material = m;
        this.itemStack = this.version.contains("1.8") || this.version.contains("1.9") || this.version.contains("1.10") || this.version.contains("1.11") || this.version.contains("1.12") ? new ItemStack(m, 1, (byte) returnData) : new ItemStack(m);
    }

    private void doPotion(ItemStack m, UPotion up) { this.itemStack = up.getItemStack(); }
    ...
    public ItemStack getItemStack() { return this.itemStack.clone(); }

    public byte getData() { return (byte) this.data; }
    ...
    private Material getMaterialName() {
        final int ver = this.version.contains("1.8") ? 0 : this.version.contains("1.9") ? 1 : this.version.contains("1.10") ? 2 : this.version.contains("1.11") ? 3 : this.version.contains("1.12") ? 4 : this.version.contains("1.13") ? 5 : this.version.contains("1.14") ? 6 : names.length - 1;
        int realver = this.names.length <= ver ? this.names.length - 1 : ver;
        if (this.names[realver] == null) {
            boolean did = false;
            for (int i = realver; i >= 0; i--) {
                if (!did && this.names[i] != null) {
                    realver = i;
                    did = true;
                }
            }
        }
        final String t = this.names[realver], t2 = this.names.length > ver ? this.names[ver] : this.names[this.names.length - 1];
    ...
    private ItemStack setItemStack(String name, int data) {
        ...
        final Material m = getMaterialName();
        this.material = m;
        return this.version.contains("1.8") || this.version.contains("1.9") || this.version.contains("1.10") || this.version.contains("1.11") || this.version.contains("1.12") ? new ItemStack(m, 1, (byte) data) : new ItemStack(m);
    }
