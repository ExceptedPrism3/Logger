...
/**
 * Logs player item drop events to the database.
 */
public class ItemDropLogger extends AbstractLogger implements DatabasePlayerLogger {

    public ItemDropLogger(final LoggerAPI plugin) { super(plugin); }

    /**
     * Logs an item drop event for a player.
     *
    ...
    public void logEvent(final Player player, final Map<String, String> placeholders) {
        ...
        final String sql = "INSERT INTO player_item_drop (date, server_name, player_uuid, player_name, world_name, location_x, location_y, location_z, item_type, item_amount, enchants, is_staff)" +
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
