...
public class SQLiteData {
    ...
    public void createTable() {
        ...
        final PreparedStatement playerChat, playerCommands, consoleCommands, playerSignText, playerJoin, playerLeave,
                ...
                wrongPassword, itemPickup, furnace, rCon, gameMode, craft, vault, registration, primedTNT,
                chestInteraction, liteBans,
        ...
        try {
            ...
            playerCommands = plugin.getSqLite().getConnection()
            ...
            playerSignText = plugin.getSqLite().getConnection()
            ...
            playerTeleport = plugin.getSqLite().getConnection()
            ...
            registration = plugin.getSqLite().getConnection()
                    .prepareStatement("CREATE TABLE IF NOT EXISTS registration" +
            ...
            chestInteraction = plugin.getSqLite().getConnection()
                    .prepareStatement("CREATE TABLE IF NOT EXISTS chest_interaction" +
            ...
            // Server Side Part
            serverStart = plugin.getSqLite().getConnection()
            ...
            consoleCommands = plugin.getSqLite().getConnection()
            ...
            portalCreation = plugin.getSqLite().getConnection()
            ...
            commandBlock = plugin.getSqLite().getConnection()
                    .prepareStatement("CREATE TABLE IF NOT EXISTS command_block" +
                            "(server_name TEXT(30), date TIMESTAMP DEFAULT CURRENT_TIMESTAMP PRIMARY KEY, command TEXT(256))");
            ...
            if (AuthMeUtil.getAuthMeAPI() != null) {
                ...
                wrongPassword = plugin.getSqLite().getConnection()
                        .prepareStatement("CREATE TABLE IF NOT EXISTS wrong_password" +
                                "(server_name TEXT(30), date TIMESTAMP DEFAULT CURRENT_TIMESTAMP PRIMARY KEY, world TEXT(100), player_name TEXT(40),"
                                +
                                "is_staff INTEGER)");
            ...
            if (AdvancedBanUtil.getAdvancedBanAPI() != null) {
                ...
                advancedBan = plugin.getSqLite().getConnection()
                        .prepareStatement("CREATE TABLE IF NOT EXISTS advanced_ban " +
            ...
            if (VersionUtil.SERVER_VERSION.isAtLeast(NmsVersions.v1_13_R1)) {
                ...
                woodStripping = plugin.getSqLite().getConnection()
                        .prepareStatement("CREATE TABLE IF NOT EXISTS wood_stripping" +
    ...
    public static void insertPlayerChat(String serverName, Player player, String message, boolean staff) {
        ...
        try {
            final PreparedStatement playerChat = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_chat (server_name, date, world, player_name, message, is_staff) VALUES (?,?,?,?,?,?)");
    ...
    public static void insertPlayerCommands(String serverName, Player player, String command, boolean staff) {
        try {
            final PreparedStatement playerCommands = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_commands (server_name, date, world, player_name, command, is_staff) VALUES (?,?,?,?,?,?)");
    ...
    public static void insertConsoleCommands(String serverName, String command) {
        try {
            final PreparedStatement consoleCommands = plugin.getSqLite().getConnection()
    ...
    public static void insertPlayerSignText(String serverName, Player player, String lines, boolean isStaff) {
        try {
            final PreparedStatement playerSignText = plugin.getSqLite().getConnection().prepareStatement(
    ...
    public static void insertPlayerDeath(String serverName, Player player, String cause, String byWho,
            boolean isStaff) {
        try {
            final PreparedStatement playerDeath = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_death (server_name, date, world, player_name, player_level, x, y, z, cause, by_who, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertPlayerTeleport(String serverName, Player player, Location from, Location to,
            boolean isStaff) {
        try {
            final PreparedStatement playerTeleport = plugin.getSqLite().getConnection().prepareStatement(
    ...
    public static void insertPlayerJoin(String serverName, Player player, boolean isStaff) {
        try {
            final PreparedStatement playerJoin = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_join (server_name, world, x, y, z, ip, is_staff) VALUES (?,?,?,?,?,?,?)");
    ...
    public static void insertPlayerLeave(String serverName, Player player, boolean isStaff) {
        try {
            final PreparedStatement playerLeave = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_leave (server_name, date, world, x, y, z, player_name, is_staff) VALUES (?,?,?,?,?,?,?,?)");
    ...
    public static void insertBlockPlace(String serverName, Player player, Material block, boolean isStaff) {
        try {
            final PreparedStatement blockPlace = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO block_place (server_name, date, world, player_name, block, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertBlockBreak(String serverName, Player player, Material block, boolean isStaff) {
        try {
            final PreparedStatement blockBreak = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO block_break (server_name, date, world, player_name, block, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertTPS(String serverName, double tps) {
        try {
            final PreparedStatement tpsStatement = plugin.getSqLite().getConnection()
                    .prepareStatement("INSERT INTO tps (server_name, date, tps) VALUES (?,?,?)");
    ...
    public static void insertRAM(String serverName, long totalMemory, long usedMemory, long freeMemory) {
        try {
            final PreparedStatement ramStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO ram (server_name, date, total_memory, used_memory, free_memory) VALUES (?,?,?,?,?)");
    ...
    public static void insertPlayerKick(String serverName, Player player, String reason, boolean isStaff) {
        try {
            final PreparedStatement playerKickStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_kick (server_name, date, world, player_name, x, y, z, reason, is_staff) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertPortalCreate(String serverName, String worldName, PortalCreateEvent.CreateReason reason) {
        try {
            final PreparedStatement portalCreateStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO portal_creation (server_name, date, world, caused_by) VALUES (?,?,?,?)");
    ...
    public static void insertLevelChange(String serverName, Player player, boolean isStaff) {
        try {
            final PreparedStatement levelChangeStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO player_level (server_name, date, player_name, is_staff) VALUES (?,?,?,?)");
    ...
    public static void insertBucketFill(String serverName, Player player, String bucket, boolean isStaff) {
        try {
            final PreparedStatement bucketFillStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO bucket_fill (server_name, date, world, player_name, bucket, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertBucketEmpty(String serverName, Player player, String bucket, boolean isStaff) {
        try {
            final PreparedStatement bucketEmptyStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO bucket_empty (server_name, date, world, player_name, bucket, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertAnvil(String serverName, Player player, String newName, boolean isStaff) {
        try {
            final PreparedStatement anvilStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO anvil (server_name, date, player_name, new_name, is_staff) VALUES (?,?,?,?,?)");
    ...
    public static void insertServerStart(String serverName) {
        try {
            final PreparedStatement serverStartStatement = plugin.getSqLite().getConnection()
                    .prepareStatement("INSERT INTO server_start (server_name, date) VALUES (?,?)");
    ...
    public static void insertServerStop(String serverName) {
        try {
            final PreparedStatement serverStopStatement = plugin.getSqLite().getConnection()
                    .prepareStatement("INSERT INTO server_stop (server_name, date) VALUES (?,?)");
    ...
    public static void insertItemDrop(String serverName, Player player, String item, int amount, int x, int y, int z,
            List<String> enchantment, String changedName, boolean isStaff) {
        try {
            final PreparedStatement itemDropStatement = plugin.getSqLite().getConnection().prepareStatement(
    ...
    public static void insertEnchant(String serverName, Player player, List<String> enchantment, String item, int cost,
            boolean isStaff) {
        try {
            final PreparedStatement enchantStatement = plugin.getSqLite().getConnection().prepareStatement(
    ...
    public static void insertBook(String serverName, Player player, int pages, List<String> content, String signedBy,
            boolean staff) {
        try {
            final PreparedStatement bookEditing = plugin.getSqLite().getConnection().prepareStatement(
    ...
    public static void insertAFK(String serverName, Player player, boolean isStaff) {
        ...
        if (EssentialsUtil.getEssentialsAPI() != null) {
            ...
            try {
                final PreparedStatement enchantStatement = plugin.getSqLite().getConnection().prepareStatement(
                        "INSERT INTO afk (server_name, date, world, player_name, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?)");
                enchantStatement.setString(1, serverName);
                enchantStatement.setString(2, dateTimeFormatter.format(ZonedDateTime.now()));
                enchantStatement.setString(3, player.getWorld().getName());
                enchantStatement.setString(4, player.getName());
    ...
    public static void insertWrongPassword(String serverName, Player player, boolean isStaff) {
        ...
        try {
            final PreparedStatement wrongPasswordStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO wrong_password (server_name, date, world, player_name, is_staff) VALUES (?,?,?,?,?)");
    ...
    public static void insertItemPickup(String serverName, Player player, Material item, int amount, int x, int y,
            int z, String changed_name, List<String> enchants, boolean isStaff) {
        try {
            final PreparedStatement itemPickupStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO item_pickup (server_name, date, world, player_name, item, amount, x, y, z, changed_name, enchantments, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertFurnace(String serverName, Player player, Material item, int amount, int x, int y, int z,
            boolean isStaff) {
        try {
            final PreparedStatement furnaceStatement = plugin.getSqLite().getConnection().prepareStatement(
    ...
    public static void insertRcon(String serverName, String command) {
        try {
            final PreparedStatement rCon = plugin.getSqLite().getConnection()
                    .prepareStatement("INSERT INTO rcon (server_name, date, Command) VALUES (?,?,?)");
    ...
    public static void insertGameMode(String serverName, Player player, String gameMode, boolean staff) {
        try {
            final PreparedStatement gameModeStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO game_mode (server_name, date, world, player_name, game_mode, is_staff) VALUES (?,?,?,?,?,?)");
    ...
    public static void insertPlayerCraft(String serverName, Player player, String item, int amount, int x, int y, int z,
            boolean isStaff) {
        try {
            final PreparedStatement craftStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO crafting (server_name, date, world, player_name, item, amount, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertVault(String serverName, Player player, double oldBal, double newBal, boolean isStaff) {
        try {
            final PreparedStatement vault = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO vault (server_name, date, player_name, old_balance, new_balance, is_staff) VALUES (?,?,?,?,?,?)");
    ...
    public static void insertRegistration(String serverName, Player player, String joinDate) {
        try {
            final PreparedStatement register = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO registration (server_name, date, player_name, player_uuid, join_date) VALUES (?,?,?,?,?)");
    ...
    public static void insertPrimedTNT(String serverName, Player player, int x, int y, int z, boolean isStaff) {
        try {
            final PreparedStatement primedTNT = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO primed_tnt (server_name, date, world, player_uuid, player_name, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertLiteBans(String serverName, String executor, String command, String onWho, String duration,
    ...
    public static void insertAdvancedBan(String serverName, String executor, String executedOn, String type,
            String reason, long expirationDate) {
        try {
            final PreparedStatement advancedBan = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO advanced_ban (server_name, date, type, executor, executed_on, reason, expiration_date) VALUES (?,?,?,?,?,?,?)");
    ...
    public static void insertCommandBlock(String serverName, String command) {
        try {
            final PreparedStatement commandBlock = plugin.getSqLite().getConnection()
                    .prepareStatement("INSERT INTO command_block (server_name, date, command) VALUES (?,?,?)");
    ...
    public static void insertWoodStripping(String serverName, Player player, String logName, int x, int y, int z,
            boolean isStaff) {
        try {
            final PreparedStatement woodStrippingStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO wood_stripping (server_name, date, world, player_uuid, player_name, log_name, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertChestInteraction(String serverName, Player player, int x, int y, int z, String[] items,
            boolean isStaff) {
        try {
            final PreparedStatement chestInteractionStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO chest_interaction (server_name, date, world, player_uuid, player_name, x, y, z, items, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertEntityDeath(String serverName, Player player, String mob, int x, int y, int z,
            boolean isStaff) {
        try {
            final PreparedStatement entityDeathStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO entity_death (server_name, date, world, player_uuid, player_name, mob, x, y, z, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void insertSignChange(String serverName, Player player, int x, int y, int z, String text,
            boolean isStaff) {
        try {
            final PreparedStatement entityDeathStatement = plugin.getSqLite().getConnection().prepareStatement(
                    "INSERT INTO sign_change (server_name, date, world, player_uuid, player_name, x, y, z, new_text, is_staff) VALUES (?,?,?,?,?,?,?,?,?,?)");
            entityDeathStatement.setString(1, serverName);
            entityDeathStatement.setString(2, dateTimeFormatter.format(ZonedDateTime.now()));
            entityDeathStatement.setString(3, player.getWorld().getName());
            entityDeathStatement.setString(4, player.getUniqueId().toString());
    ...
    public void emptyTable() {
        ...
        try {
            ...
            // Player Side Part
            final PreparedStatement playerChat = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_chat WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerCommands = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_commands WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerSignText = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_sign_text WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerJoin = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_join WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerLeave = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_leave WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerKick = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_kick WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerDeath = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_death WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerTeleport = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_teleport WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement playerLevel = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM player_level WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement blockPlace = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM block_place WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement blockBreak = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM block_break WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement bucketFill = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM bucket_fill WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement bucketEmpty = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM bucket_empty WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement anvil = plugin.getSqLite().getConnection()
                    .prepareStatement("DELETE FROM anvil WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement itemDrop = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM item_drop WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement enchanting = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM enchanting WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement bookEditing = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM book_editing WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement itemPickup = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM item_pickup WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement furnace = plugin.getSqLite().getConnection()
                    .prepareStatement("DELETE FROM furnace WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement gameMode = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM game_mode WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement craft = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM crafting WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement register = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM registration WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement primedTNT = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM primed_tnt WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement chestInteraction = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM chest_interaction WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement entityDeath = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM entity_death WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement signChange = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM sign_change WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            // Server Side Part
            final PreparedStatement serverStart = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM server_start WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement serverStop = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM server_stop WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement consoleCommands = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM console_commands WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement ram = plugin.getSqLite().getConnection()
                    .prepareStatement("DELETE FROM ram WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement tps = plugin.getSqLite().getConnection()
                    .prepareStatement("DELETE FROM tps WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement portalCreation = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM portal_creation WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement rCon = plugin.getSqLite().getConnection()
                    .prepareStatement("DELETE FROM rcon WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            final PreparedStatement commandBlock = plugin.getSqLite().getConnection().prepareStatement(
                    "DELETE FROM command_block WHERE date <= datetime('now','-" + sqliteDataDel + " day')");

            // Extras Side Part
            if (EssentialsUtil.getEssentialsAPI() != null) {

                final PreparedStatement afk = plugin.getSqLite().getConnection()
                        .prepareStatement("DELETE FROM afk WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
            ...
            if (AuthMeUtil.getAuthMeAPI() != null) {
                ...
                final PreparedStatement wrongPassword = plugin.getSqLite().getConnection().prepareStatement(
                        "DELETE FROM wrong_password WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
            ...
            if (VaultUtil.getVaultAPI() && VaultUtil.getVault() != null) {
                ...
                final PreparedStatement vault = plugin.getSqLite().getConnection().prepareStatement(
                        "DELETE FROM vault WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
            ...
            if (LiteBansUtil.getLiteBansAPI() != null) {
                ...
                final PreparedStatement liteBans = plugin.getSqLite().getConnection().prepareStatement(
                        "DELETE FROM litebans WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
            ...
            if (AdvancedBanUtil.getAdvancedBanAPI() != null) {
                ...
                final PreparedStatement advancedBan = plugin.getSqLite().getConnection().prepareStatement(
                        "DELETE FROM advanced_ban WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
            ...
            if (VersionUtil.SERVER_VERSION.isAtLeast(NmsVersions.v1_13_R1)) {
                ...
                final PreparedStatement woodStripping = plugin.getSqLite().getConnection().prepareStatement(
                        "DELETE FROM wood_stripping WHERE date <= datetime('now','-" + sqliteDataDel + " day')");
