...
/**
 * Logs player leave events to the database.
 */
public class LeaveLogger extends AbstractLogger implements DatabasePlayerLogger {

    public LeaveLogger(final LoggerAPI plugin) { super(plugin); }

    /**
     * Logs a leave event for a player.
     *
    ...
    public void logEvent(final Player player, final Map<String, String> placeholders) {
        ...
        final String sql = "INSERT INTO player_leave (date, server_name, player_uuid, player_name, world_name, location_x, location_y, location_z, is_staff)" +
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
