...
public class Dump implements SubCommand {
    ...
    @Override
    public void perform(final CommandSender commandSender, final String[] args) throws IOException { this.pastebinExecution(commandSender); }

    @Override
    public List<String> getSubCommandsArgs(final CommandSender commandSender, final String[] args) { return Collections.emptyList(); }

    private void pastebinExecution(final CommandSender commandSender) throws IOException {

        final Dotenv dotenv = Dotenv.load();

        final String config = Files.asCharSource(new File(this.dataFolder + "/config.yml"), StandardCharsets.UTF_8).read();
        final String discord = Files.asCharSource(new File(this.dataFolder + "/discord.yml"), StandardCharsets.UTF_8).read();
        final String messages = Files.asCharSource(new File(this.dataFolder + "/messages/" + selectedLang + ".yml"), StandardCharsets.UTF_8).read();
