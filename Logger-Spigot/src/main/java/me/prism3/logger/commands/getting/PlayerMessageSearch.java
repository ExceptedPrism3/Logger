...
public class PlayerMessageSearch {
    ...
    public void fetchAndSendResults() {
        ...
        this.sendResults((List<AbstractAction>) Main.getInstance().getDatabase()
                .getSaladeMarocaine(this.searchedPlayer, this.pager.getCurrentOffset(), 10));

        if (this.pager.getCurrentOffset() > this.pager.getCount()) {
            this.sender.sendMessage("No results found for that page!");
            return;
        }

        this.sender.spigot().sendMessage(this.pager.preparePaging(this.commandLabel, this.searchedPlayer));
    }
